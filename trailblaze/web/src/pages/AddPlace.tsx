...
export default function AddPlace() {
  ...
  if (!user) return (
    <div className="flex min-h-[80vh] flex-col items-center justify-center px-5 text-center">
      ...
      <p className="mt-3 font-bold">Sign in to add places</p>
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const result = await api.places.add({ name, description, category: category.toLowerCase() as Category, county, lat: coords.lat, lng: coords.lng, images: imageUrls });
  ...
  <select value={category} onChange={(e) => setCategory(e.target.value as Category)} className="input">
    ...
    {CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
        ...
        <Field label="Description">
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={4} placeholder="What makes this place special?" className="input resize-none" />
        </Field>
        ...
        <Field label={`Images (${images.length}/${MAX_IMAGES} · min ${MIN_IMAGES})`}>
          <div className="grid grid-cols-3 gap-2">
            {images.map((img, i) => (
              <div key={i} className="relative aspect-square overflow-hidden rounded-xl border border-border">
                ...
                <button type="button" onClick={() => removeImage(i)} className="absolute right-1 top-1 flex h-6 w-6 items-center justify-center rounded-full bg-black/70 text-white">
          ...
          {images.length < MIN_IMAGES && <p className="mt-1 text-xs text-muted-foreground">Add at least {MIN_IMAGES - images.length} more image(s)</p>}
        </Field>

        <Field label="Location">
          <div className="mb-3 flex rounded-full border border-border bg-secondary p-1 text-sm font-semibold">
            <button type="button" onClick={() => setLocMode("current")} className={`flex-1 rounded-full py-2 transition ${locMode === "current" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>
              <MapPin className="mr-1 inline h-4 w-4" /> Current
            </button>
            <button type="button" onClick={() => setLocMode("pin")} className={`flex-1 rounded-full py-2 transition ${locMode === "pin" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>
              <Pin className="mr-1 inline h-4 w-4" /> Drop Pin
            </button>
          </div>
          {locMode === "current" ? (
            <button type="button" onClick={useMyLocation} className="flex w-full items-center justify-center gap-2 rounded-full bg-info py-3 text-sm font-semibold text-info-foreground">
          ...
          <p className="mt-2 text-center text-xs text-muted-foreground">📍 {coords.lat.toFixed(5)}, {coords.lng.toFixed(5)}</p>
        </Field>
      </div>
      ...
      <button type="submit" disabled={!valid || uploading}
        ...
        {uploading ? <span className="flex items-center justify-center gap-2"><Loader2 className="h-4 w-4 animate-spin" /> Uploading...</span> : "Add Place · +10 pts 🎉"}
      </button>

      {toast && <div className="fixed bottom-24 left-1/2 -translate-x-1/2 rounded-2xl bg-primary px-6 py-3 text-sm font-bold text-primary-foreground shadow-lg">{toast}</div>}
    </form>
  );
}
